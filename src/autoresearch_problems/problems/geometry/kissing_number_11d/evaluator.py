"""Evaluator for the Kissing Number problem in dimension 11.
...
# Known upper bound for dimension 11 (from Delsarte's linear programming bound)
_UPPER_BOUND_11D = 868


def evaluate(output: object, dimension: int = 11, **kwargs) -> dict:
    ...
    dimension : int
        The dimension of the space (default 11).
    ...
    if n > _UPPER_BOUND_11D:
        return {"score": 0.0, "valid": False,
                "error": f"Number of points ({n}) exceeds known upper bound ({_UPPER_BOUND_11D})",
                "metrics": {}}
