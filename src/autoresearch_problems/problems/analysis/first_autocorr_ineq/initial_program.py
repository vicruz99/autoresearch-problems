...
def solve() -> list:
    """Search for a non-negative sequence that minimizes the C₁ constant."""
    best_sequence = [np.random.random()] * np.random.randint(100, 1000)
    h_function = _get_good_direction_to_move_into(best_sequence)
...
def _get_good_direction_to_move_into(sequence):
    n = len(sequence)
